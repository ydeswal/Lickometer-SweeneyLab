...
void make_file() {
  ...
  if (log_LN) {
    for (int i = 1; i <= ACTIVE_SIPPER_COUNT; i++) {
...
void write_to_file_by_time() {
  ...
  logfile.print('"');
  if (log_LN) {
    for (int k = 0; k < ACTIVE_SIPPER_COUNT; k++) {  //write data to file for each sipper
      logfile.print(F(", "));
      logfile.print(snap_LickNumber[k]);
    }
  }
  ...
  if (log_LD) {
    for (int k = 0; k < ACTIVE_SIPPER_COUNT; k++) {  //write data to file for each sipper
      logfile.print(F(", "));
      logfile.print(snap_LickDuration[k]);
    }
  }
  lick_sample_and_update();
  if (log_BN) {
    for (int k = 0; k < ACTIVE_SIPPER_COUNT; k++) {  //write data to file for each sipper
      logfile.print(F(", "));
      logfile.print(snap_BoutNumber[k]);
    }
  }
  if (log_BD) {
    for (int k = 0; k < ACTIVE_SIPPER_COUNT; k++) {  //write data to file for each sipper
      logfile.print(F(", "));
      logfile.print(snap_BoutDuration[k]);
    }
  }
  lick_sample_and_update();
  if (log_BLN) {
    for (int k = 0; k < ACTIVE_SIPPER_COUNT; k++) {  //write data to file for each sipper
      logfile.print(F(", "));
      logfile.print(snap_BoutLickNumber[k]);
    }
  }
  if (log_BLD) {
    for (int k = 0; k < ACTIVE_SIPPER_COUNT; k++) {  //write data to file for each sipper
      logfile.print(F(", "));
      logfile.print(snap_BoutLickDuration_bytime[k]);
    }
  }
