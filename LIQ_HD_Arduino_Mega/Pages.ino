...
void main_menu_page() {
  while (display_page == "main") {  //show main menu screen
    ...
    if (refresh_page) {  //do this only once every time main menu is opened
      ...
      drawBitmap(0, 40, main_logo, 240, 51, THEME_TEXT);
      drawBitmap(8, 28, settings_icon, 32, 32, THEME_TEXT);

      tft.setFont(&FreeSansBold9pt7b);
      NickText_center("Experimental Side", 120, 110, 1);
      tft.setFont(&FreeSans9pt7b);
      Button_center(60, 150, 80, 40, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, "LEFT", 1);
      Button_center(180, 150, 80, 40, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, "RIGHT", 1);
      tft.setFont(&FreeSansBold12pt7b);
      Button_center(120, 220, 140, 60, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, "START!", 1);  //"Start recording" button --> go to recording_page (display_page = 2)

      tft.setFont(&FreeSans9pt7b);  //Experimental Side --> left or right?
      if (E_side == "LEFT") {
        Button_center(60, 150, 80, 40, THEME_SURFACE, THEME_ACCENT, THEME_TEXT, "LEFT", 1);
      }

      if (E_side == "RIGHT") {
        Button_center(180, 150, 80, 40, THEME_SURFACE, THEME_ACCENT, THEME_TEXT, "RIGHT", 1);
      }
      ...
      if (!SD.begin(chipSelect)) {  //Info if SD card is instered, if not, --> display button to mount SD card to insert and retest
        ...
        eject_button = false;
        Button_center(190, 290, 90, 30, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, "Mount SD", 1);
      }
      //error(F("Card failed, or not present"));
        ...
        eject_button = true;
        //start_SD = false;
        Button_center(190, 290, 90, 30, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, "Eject SD", 1);
      }
    }
    ...
    if (ts.touched()) {
      ...
      if (p.x > 50 && p.x < 190 && p.y > 190 && p.y < 250) {  //Start button coordiantes
        ...
        tft.drawRoundRect(50, 190, 140, 60, 4, THEME_ACCENT);
        reset_variables();
      ...
      if (p.x > 20 && p.x < 100 && p.y > 130 && p.y < 170) {    //Experimental LEFT button coordiantes
        ...
        tft.drawRoundRect(20, 130, 80, 40, 4, THEME_ACCENT);   // highlight with green border
        tft.drawRoundRect(140, 130, 80, 40, 4, THEME_SURFACE);  // unhighlight other button
        wait(100);
      }
      ...
      if (p.x > 140 && p.x < 220 && p.y > 130 && p.y < 170) {   //Experimental RIGHT button coordiantes
        ...
        tft.drawRoundRect(20, 130, 80, 40, 4, THEME_SURFACE);   // highlight with green border
        tft.drawRoundRect(140, 130, 80, 40, 4, THEME_ACCENT);  // unhighlight other button
        wait(100);
      }

      if (p.x > 145 && p.x < 235 && p.y > 275 && p.y < 305 && eject_button) {  //Eject Button button coordiantes
        p = {};                                                                //debounce touchscreen
        tft.drawRoundRect(145, 275, 90, 30, 4, THEME_ACCENT);
        SD.end();  // Eject SD card
        tft.fillRect(0, 270, 140, 30, THEME_BG);
        eject_button = false;
        wait(1000);
      ...
      if (p.x > 0 && p.x < 42 && p.y > 20 && p.y < 70) {  //Settings button button coordiantes
        p = {};                                           //debounce touchscreen
        drawBitmap(8, 28, settings_icon, 32, 32, THEME_ACCENT);
        wait(500);
...
void recording_page() {
  while (display_page == "recording") {
    ...
    if (refresh_page) {  //do this only once every time coming to recording page
      ...
      if (!cap.begin(ACTIVE_MPR121_ADDRESS)) {
        error("MPR121 #1 not found, check wiring?");
        while (!cap.begin(ACTIVE_MPR121_ADDRESS)) {
          resetWire();
          if (ts.touched()) {
            TS_Point p = ts.getPoint();
            wait(100);
      ...
      if (false) { // disabled: only one MPR121 is connected
        error("MPR121 #2 not found, check wiring?");
        while (false) { // disabled: only one MPR121 is connected
          resetWire();
          if (ts.touched()) {
            TS_Point p = ts.getPoint();
            wait(100);
      ...
      if (cap.begin(ACTIVE_MPR121_ADDRESS)) {
        set_sensor_settings();     //charge current, filters, hardware thresholds
        pair_raw_calibrated = false;
        calibrate_pair_sensors();  //measure idle level + noise, set detection thresholds
        ...
        Button_center(57, 280, 90, 40, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, "PAUSE", 1);
        Button_center(170, 280, 120, 40, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, "Save & Quit", 1);
    ...
    if (log_by_time && start_new_session) {
      reset_variables();          //full reset only when a session actually begins
      start_new_session = false;  //later bins hand over via subtract_snapshot()
    }
    ...
    while (LOG_COUNTER < (prev_min + LOG_INTERVAL)) {
      ...
      Record_Licks();  //sample the sensor and update lick/bout state
      ...
      if (ts_poll()) {
        ...
        if (p.x > 12 && p.x < 102 && p.y > 260 && p.y < 300) {  //PAUSE button coordiantes --> write leftover data to card and eject so you can safely remove SD card --> go to pause_page (display_page = "pause")
          p = {};                                               //debounce touchscreen
          Button_center(57, 280, 90, 40, THEME_SURFACE, THEME_ACCENT, THEME_TEXT, "PAUSE", 1);
          logfile.flush();
        ...
        if (p.x > 110 && p.x < 230 && p.y > 260 && p.y < 300) {                                             //Save & Quit button coordiantes --> write leftover data to card --> return to main_menu_page (display_page = "main")
          p = {};                                                                                           //debounce touchscreen
          Button_center(170, 280, 120, 40, THEME_SURFACE, THEME_ACCENT, THEME_TEXT, "Save & Quit", 1);  // highlight with green border
          logfile.flush();
          DEBUG_PRINT("Writing data to SD card!");
          logfile.close();
          SD.end();
      ...
      Record_Licks();  //second sample point, keeps coverage during screen work

      curr_min = cached_minute();
    ...
    if (log_by_time) {
      ...
      log_time_bin();     //snapshot -> write row -> update totals -> subtract snapshot
    }
...
void pause_page() {
  while (display_page == "pause") {
    ...
    if (refresh_page) {
      ...
      tft.setFont(&FreeSans12pt7b);
      Button_center(120, 250, 110, 40, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, "Resume", 1);
    }
    ...
    if (ts.touched()) {
      ...
      if (p.x > 65 && p.x < 175 && p.y > 230 && p.y < 270) {  //Resume button coordiantes, Unpause --> mount SD card and continue recording to the same file --> go to recording_page (display_page = "recording")
        p = {};                                               //debounce touchscreen
        tft.drawRoundRect(65, 230, 110, 40, 4, THEME_ACCENT);
        wait(100);
  ...
  while (display_page == "SDerror") {
    ...
    if (ts.touched()) {
      ...
      if (p.x > 10 && p.x < 230 && p.y > 220 && p.y < 280) {  //Return to Main Menu button coordiantes // fix this
        p = {};                                               //debounce touchscreen
        Button_center(120, 250, 220, 60, THEME_SURFACE, THEME_ACCENT, THEME_TEXT, "Return to Main Menu", 1);
        wait(100);
        display_page = "main";  // go to main_menu_page
        refresh_page = true;
      }
...
void settings_page() {
  while (display_page == "settings") {
    ...
    if (refresh_page) {
      ...
      Button_center(130, 80, 60, 26, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, "Edit", 1);
      NickText("Lights ON Time", 10, 115, 1);
      Button_center(160, 110, 35, 26, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, lights_on, 1);
      NickText("Lights OFF Time", 10, 145, 1);
      Button_center(168, 140, 35, 26, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, lights_off, 1);
      NickText("Sensor Settings", 10, 175, 1);
      Button_center(175, 170, 60, 26, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, "Edit", 1);
      NickText("Parameters to Log", 10, 205, 1);
      Button_center(195, 200, 60, 26, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, "Edit", 1);
      if (log_by_time) {
        NickText("Time Bin Size", 10, 235, 1);
        Button_center(145, 230, 35, 26, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, LOG_INTERVAL, 1);
      }
      NickText("Sync Interval", 10, 265, 1);
      Button_center(137, 260, 35, 26, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, SYNC_INTERVAL, 1);
      Button_center(180, 295, 60, 40, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, "Save", 1);
      Button_center(60, 295, 60, 40, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, "Reset", 1);
    }
    ...
    if (ts.touched()) {
      ...
      if (p.x > 100 && p.x < 160 && p.y > 67 && p.y < 93) {  //Date/Time Edit
        p = {};                                              //debounce touchscreen
        Button_center(130, 80, 60, 26, THEME_SURFACE, THEME_ACCENT, THEME_TEXT, "Edit", 1);
        wait(100);
      ...
      if (p.x > 143 && p.x < 178 && p.y > 97 && p.y < 123) {  //Lights ON
        p = {};                                               //debounce touchscreen
        Button_center(160, 110, 35, 26, THEME_SURFACE, THEME_ACCENT, THEME_TEXT, lights_on, 1);
        wait(100);
      ...
      if (p.x > 151 && p.x < 186 && p.y > 127 && p.y < 153) {  //Lights OFF
        p = {};                                                //debounce touchscreen
        Button_center(168, 140, 35, 26, THEME_SURFACE, THEME_ACCENT, THEME_TEXT, lights_off, 1);
        wait(100);
      ...
      if (p.x > 145 && p.x < 205 && p.y > 157 && p.y < 183) {  //Sensor Settings Edit
        p = {};                                                //debounce touchscreen
        Button_center(175, 170, 60, 26, THEME_SURFACE, THEME_ACCENT, THEME_TEXT, "Edit", 1);
        wait(100);
      ...
      if (p.x > 165 && p.x < 225 && p.y > 187 && p.y < 213) {  //Parameters to log Edit
        p = {};                                                //debounce touchscreen
        Button_center(195, 200, 60, 26, THEME_SURFACE, THEME_ACCENT, THEME_TEXT, "Edit", 1);
        wait(100);
      ...
      if (p.x > 128 && p.x < 163 && p.y > 217 && p.y < 243 && log_by_time) {  //Time bin size
        p = {};                                                //debounce touchscreen
        Button_center(145, 230, 35, 26, THEME_SURFACE, THEME_ACCENT, THEME_TEXT, LOG_INTERVAL, 1);
        wait(100);
      ...
      if (p.x > 120 && p.x < 155 && p.y > 247 && p.y < 273) {  //Sync interval
        p = {};                                                //debounce touchscreen
        Button_center(137, 260, 35, 26, THEME_SURFACE, THEME_ACCENT, THEME_TEXT, SYNC_INTERVAL, 1);
        wait(100);
      ...
      if (p.x > 150 && p.x < 210 && p.y > 275 && p.y < 315) {  //Save
        p = {};                                                //debounce touchscreen
        Button_center(180, 295, 60, 40, THEME_SURFACE, THEME_ACCENT, THEME_TEXT, "Save", 1);
        wait(100);
        display_page = "main";  // go to main_menu_page
        refresh_page = true;
        show_time = true;
      ...
      if (p.x > 30 && p.x < 90 && p.y > 275 && p.y < 315) {  //Reset
        p = {};                                              //debounce touchscreen
        Button_center(60, 295, 60, 40, THEME_SURFACE, THEME_ACCENT, THEME_TEXT, "Reset", 1);
        wait(100);
        lights_on = default_lights_on;
        lights_off = default_lights_off;
  ...
  while (display_page == "settings_datetime") {
    ...
    if (refresh_page) {
      ...
      Button_center(90, 80, 35, 26, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, now.month(), 1);
      NickText("Day", 10, 115, 1);
      Button_center(90, 110, 35, 26, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, now.day(), 1);
      NickText("Year", 10, 145, 1);
      Button_center(90, 140, 60, 26, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, now.year(), 1);
      NickText("Hour", 10, 205, 1);
      Button_center(90, 200, 35, 26, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, now.hour(), 1);
      NickText("Minute", 10, 235, 1);
      Button_center(90, 230, 35, 26, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, now.minute(), 1);
      Button_center(180, 295, 60, 40, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, "Save", 1);
    }
    ...
    if (ts.touched()) {
      ...
      if (p.x > 73 && p.x < 108 && p.y > 67 && p.y < 93) {  //month
        p = {};                                             //debounce touchscreen
        Button_center(90, 80, 35, 26, THEME_SURFACE, THEME_ACCENT, THEME_TEXT, now.month(), 1);
        wait(100);
      ...
      if (p.x > 73 && p.x < 108 && p.y > 97 && p.y < 123) {  //day
        p = {};                                              //debounce touchscreen
        Button_center(90, 110, 35, 26, THEME_SURFACE, THEME_ACCENT, THEME_TEXT, now.day(), 1);
        wait(100);
        int new_day = now.day();
        GetNum("Day", new_day);
        if (new_day > 31) {
      ...
      if (p.x > 60 && p.x < 120 && p.y > 127 && p.y < 153) {  //year
        p = {};                                               //debounce touchscreen
        Button_center(90, 140, 60, 26, THEME_SURFACE, THEME_ACCENT, THEME_TEXT, now.year(), 1);
        wait(100);
      ...
      if (p.x > 73 && p.x < 108 && p.y > 187 && p.y < 213) {  //hour
        p = {};                                               //debounce touchscreen
        Button_center(90, 200, 35, 26, THEME_SURFACE, THEME_ACCENT, THEME_TEXT, now.hour(), 1);
        wait(100);
      ...
      if (p.x > 73 && p.x < 108 && p.y > 217 && p.y < 243) {  //minute
        p = {};                                               //debounce touchscreen
        Button_center(90, 230, 35, 26, THEME_SURFACE, THEME_ACCENT, THEME_TEXT, now.minute(), 1);
        wait(100);
  ...
  while (display_page == "settings_parameters") {
    ...
    if (refresh_page) {
      ...
      if (log_by_time) {
        ...
        tft.setFont(&FreeSansBold12pt7b);
        Button_center(22, 80, 24, 24, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, logging[0], 1);

        tft.setFont(&FreeSans9pt7b);
        NickText("Lick Duration", 44, 115, 1);
        tft.setFont(&FreeSansBold12pt7b);
        Button_center(22, 110, 24, 24, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, logging[1], 1);
      }

      tft.setFont(&FreeSans9pt7b);
      NickText("Bout Number", 44, 145, 1);
      tft.setFont(&FreeSansBold12pt7b);
      Button_center(22, 140, 24, 24, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, logging[2], 1);

      tft.setFont(&FreeSans9pt7b);
      NickText("Bout Duration", 44, 175, 1);
      tft.setFont(&FreeSansBold12pt7b);
      Button_center(22, 170, 24, 24, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, logging[3], 1);

      tft.setFont(&FreeSans9pt7b);
      NickText("Bout Lick Number", 44, 205, 1);
      tft.setFont(&FreeSansBold12pt7b);
      Button_center(22, 200, 24, 24, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, logging[4], 1);

      tft.setFont(&FreeSans9pt7b);
      NickText("Bout Lick Duration", 44, 235, 1);
      tft.setFont(&FreeSansBold12pt7b);
      Button_center(22, 230, 24, 24, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, logging[5], 1);

      tft.setFont(&FreeSans9pt7b);
      NickText("Log byTime", 29, 265, 1);
      tft.setFont(&FreeSansBold12pt7b);
      Button_center(14, 260, 24, 24, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, logging[6], 1);

      tft.setFont(&FreeSans9pt7b);
      NickText("Log Bouts", 155, 265, 1);
      tft.setFont(&FreeSansBold12pt7b);
      Button_center(140, 260, 24, 24, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, logging[7], 1);
  ...
  while (display_page == "settings_sensor") {
    ...
    if (refresh_page) {
      ...
      Button_center(170, 80, 35, 26, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, touch_threshold, 1);
      NickText("Release Threshold", 10, 115, 1);
      Button_center(185, 110, 35, 26, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, release_threshold, 1);
      NickText("Auto Calibrate?", 10, 145, 1);
      tft.setFont(&FreeSansBold12pt7b);
      Button_center(150, 140, 24, 24, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, X, 1);
      tft.setFont(&FreeSans9pt7b);
      if (auto_cal) {
        NickText("Time (24hr)", 30, 175, 1);
        Button_center(150, 170, 35, 26, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, auto_cal_time, 1);
        NickText("Time since last lick (sec) ", 30, 205, 1);
        Button_center(120, 225, 40, 26, THEME_SURFACE, THEME_SURFACE, THEME_TEXT, auto_cal_sec_since_last_lick, 1);
        tft.setFont();
    ...
    if (ts.touched()) {
      ...
      if (p.x > 153 && p.x < 188 && p.y > 67 && p.y < 93) {  //touch threshold
        p = {};                                              //debounce touchscreen
        tft.setFont(&FreeSans9pt7b);
        Button_center(170, 80, 35, 26, THEME_SURFACE, THEME_ACCENT, THEME_TEXT, touch_threshold, 1);
        wait(100);
      ...
      if (p.x > 168 && p.x < 203 && p.y > 97 && p.y < 123) {  //release threshold
        p = {};                                               //debounce touchscreen
        tft.setFont(&FreeSans9pt7b);
        Button_center(185, 110, 35, 26, THEME_SURFACE, THEME_ACCENT, THEME_TEXT, release_threshold, 1);
        wait(100);
      ...
      if (p.x > 133 && p.x < 168 && p.y > 157 && p.y < 183 && auto_cal) {  //Time to cal
        p = {};                                                            //debounce touchscreen
        tft.setFont(&FreeSans9pt7b);
        Button_center(150, 170, 35, 26, THEME_SURFACE, THEME_ACCENT, THEME_TEXT, auto_cal_time, 1);
        wait(100);
      ...
      if (p.x > 85 && p.x < 155 && p.y > 217 && p.y < 243 && auto_cal) {  //time since last cal
        p = {};                                                           //debounce touchscreen
        tft.setFont(&FreeSans9pt7b);
        Button_center(120, 225, 40, 26, THEME_SURFACE, THEME_ACCENT, THEME_TEXT, auto_cal_sec_since_last_lick, 1);
        wait(100);
