...
// ==========================================================================================
bool auto_threshold = true;             // set thresholds from measured noise at calibration
float auto_threshold_sd_multiple = 5.0; // threshold = this many standard deviations of idle noise
int  min_touch_threshold = 4;           // never go below this, even on a very quiet rig
int  max_touch_threshold = 10;          // if auto lands above this your hardware needs work
...
// ==========================================================================================
//  SECTION 4b - SEPARATING INDIVIDUAL LICKS INSIDE A BOUT
//
//  This section exists because of one specific failure: during continuous licking,
//  the reading does not come all the way back to the idle baseline between tongue
//  contacts. The animal's snout is right at the spout, liquid bridges the gap, and
//  the body adds a steady DC offset for as long as it stays there.
//
//  If that residual offset never falls below release_threshold, the detector never
//  releases, and a whole run of licks is recorded as ONE long touch. Bout timing
//  still looks correct - the bout starts at the first contact and ends at the last -
//  but the lick count inside the bout collapses. That is the "bouts are accurate,
//  individual licks are not" symptom exactly.
//
//  Two independent mechanisms fix it. Both can be switched off to compare.
//
//  1. PLATEAU TRACKER (AC coupling).
//     Estimates the sustained DC component of the signal and subtracts it, so each
//     lick is measured from the current local floor rather than from the idle level.
//     Rises slowly (a lick must not be tracked out) and falls quickly (the floor
//     between contacts must be re-acquired inside one inter-lick gap).
//
//  2. RELATIVE RELEASE.
//     Releases when the signal has fallen to a fraction of this contact's own peak,
//     not only when it reaches an absolute floor. A tongue withdrawal is a large
//     fractional drop even when it sits on top of a DC offset.
// ==========================================================================================
...
int  plateau_max = 400;        // sanity clamp on the DC estimate, in counts

bool use_relative_release = true;
int  release_drop_percent = 40;   // release once the signal falls to this % of the contact's peak
int  relative_release_min_peak = 2;  // only apply it when peak >= this multiple of touch_threshold
int  relative_release_samples = 2;   // dip must persist this many samples (rejects single-sample noise)
...
int32_t  lk_plateau_q[LK_NCH];       // DC estimate, same fixed point as the baseline
int16_t  lk_plateau[LK_NCH];         // DC estimate in plain counts, for the readout
int16_t  lk_dc_raw[LK_NCH];          // delta before plateau subtraction
uint8_t  lk_relrel_count[LK_NCH];    // consecutive samples satisfying relative release
unsigned long lk_merged_saved[LK_NCH];  // licks recovered by relative release
unsigned long lk_last_onset[LK_NCH];    // for inter-lick interval reporting

// Snapshot of the counters taken at each time bin, so logging never loses a lick.
unsigned long snap_LickNumber[LK_NCH];
unsigned long snap_LickDuration[LK_NCH];
unsigned long snap_BoutNumber[LK_NCH];
unsigned long snap_BoutDuration[LK_NCH];
unsigned long snap_BoutLickNumber[LK_NCH];
unsigned long snap_BoutLickDuration[LK_NCH];
unsigned long snap_BoutLickDuration_bytime[LK_NCH];
bool start_new_session = true;
...
// ==========================================================================================
//  SECTION 7 - SCREEN THEME   (Sweeney Lab / LIQ HD, by YD)
//
//  Every colour the interface uses is defined here. Change a value and it changes
//  everywhere - buttons, headers, keypad, settings pages, all of it. Colours are
//  RGB565: red in the top 5 bits, green in the middle 6, blue in the bottom 5.
//
//  There is an online picker at rinkydinkelectronics.com/calc_rgb565.php, or work it
//  out yourself:  value = ((R & 0xF8) << 8) | ((G & 0xFC) << 3) | (B >> 3)
// ==========================================================================================
#define THEME_BG        0x0884   // #0B1220  page background
#define THEME_SURFACE   0x1948   // #1B2B45  panels, buttons, header bar
#define THEME_TEXT      0xEF9F   // #EAF2FF  primary text and icons
#define THEME_ACCENT    0x2E98   // #2ED3C6  highlight and pressed state
#define THEME_MUTED     0x8D17   // #8FA3BE  secondary labels
#define THEME_LEFT      0x4D5F   // #4EA8FF  left bottle
#define THEME_RIGHT     0xFDA9   // #FFB44E  right bottle
#define THEME_WARN      0xFAEA   // #FF5F56  errors and warnings

// Alternative palettes. Comment out the block above and uncomment one of these.
//
// WARM DARK
//   #define THEME_BG 0x1082
//   #define THEME_SURFACE 0x28E3
//   #define THEME_TEXT 0xFF9C
//   #define THEME_ACCENT 0xED07
//   #define THEME_MUTED 0xAC6E
//   #define THEME_LEFT 0x7D9A
//   #define THEME_RIGHT 0xED07
//   #define THEME_WARN 0xFAEA
//
// HIGH CONTRAST (best for a dim room or a dark cycle)
//   #define THEME_BG 0x0000
//   #define THEME_SURFACE 0x2104
//   #define THEME_TEXT 0xFFFF
//   #define THEME_ACCENT 0x07EF
//   #define THEME_MUTED 0xA514
//   #define THEME_LEFT 0x05FF
//   #define THEME_RIGHT 0xFEA0
//   #define THEME_WARN 0xF986

// Header text shown on the recording screen. Keep these short, they are centred.
...
// Colour the left/right totals by bottle instead of plain white.
bool colour_code_bottles = true;
